%zadanie 7 - Skrypt relizujacy algorytm DMC regulatora rozmytego
...
[D,N,Nu,lambda] = nastawy_DMC(fuzzyN);%nastawy dla opd. ilosci regulatorow
settings = [];
...
for n=1:fuzzyN
    ...
    settings(n).N = N(n);
    settings(n).Nu = Nu(n);
    settings(n).lambda = lambda(n);
    [Mp,K,u_delta] = parametry_DMC(settings(n).D,settings(n).N,settings(n).Nu,settings(n).lambda,fuzzyN,n);
    settings(n).Mp = Mp;
    settings(n).K = K;
...
ylabel('u')
xlabel('k');
dlmwrite('../Dane/Zad7/N5/u.csv', u, '\t');
dlmwrite('../Dane/Zad7/N5/y.csv', y_mod, '\t');
dlmwrite('../Dane/Zad7/N5/y_zad.csv', y_zad, '\t');