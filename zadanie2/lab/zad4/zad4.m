...
%wczytanie aproksymowanych odpowiedzi skokowych
load('../zad3/s_skok_s_32_55_approx.mat');
load('../zad3/s_skok_z_0_30_approx.mat');
% gorne ograniczenie sterowania
load('u_max.mat');
...
start = 300; %start symulacji
start_z = 1000; %start symulacji
...
% Obliczanie parametrow regulatora
...
K = ((M'*psi*M+mac_lam)^(-1))*M'*psi;
 ...
 %% ustalenie poczatkowych sterowan, W1=50, G1=U
...
for k = D:kk
    ...
    % przesuniecie wektora delta u 
    for n = D-1:-1:2
       u_delta(n) = u_delta(n-1);
    end

    % jesli istnieja zaklocenia i uwzgledniamy je
    % przesuniecie wektora delta z
    if zakl == 1 && licz_z == 1
       for n = Dz:-1:2
    ...
    %wykres wyjscia i wartosci zadanej
    figure(1);
    clf(1);
    hold on;
    title('y');
...
E
%Wyswietlenie wynikow po zakonczeniu petli sterowania
%wyniki
figure;
subplot(3,1,1);
hold on;
...
%zapisanie danych sterowania do plikow
if licz_z == 1
